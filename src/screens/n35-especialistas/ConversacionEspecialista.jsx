// N-35 · MUVET Especialistas — el hilo 1:1, común a las dos mitades.
//
// La negociación es idéntica venga del directorio o del tablón (`origen` solo
// cambia el encabezado de contexto), así que hay una sola pantalla:
//
//   1. Chat EN TIEMPO REAL con ADJUNTOS (bucket privado `especialista-chat`).
//      El adjunto no es un extra: entre profesionales se mandan exámenes,
//      radiografías y cotizaciones.
//   2. El chat sigue ABIERTO tras el acuerdo y se cierra al FINALIZAR el
//      servicio. Lo impone la policy de insert de 0039, no esta pantalla.
//   3. Cierre por acuerdo mutuo: cada parte marca su bandera y `estado` lo
//      deriva el trigger. El cliente nunca escribe `estado`.
//   4. Al finalizar NO se borra nada: el historial queda para releerlo desde
//      N-9 y N-34.
//
// Y una cosa que NO está a propósito: ningún teléfono, ni antes ni después del
// acuerdo. Tampoco panel de pagos — el especialista le cobra directo a quien lo
// contrata, no hay pago entre las partes que la app deba marcar.
...
import FichaContactoEspecialista from './FichaContactoEspecialista';
import { validateResultFile } from '../../lib/fileValidation';
import { formatCOP } from '../../lib/format';
import { NOMBRE_ESPECIALISTAS } from '../../lib/nombresModulos';
import {
  fetchConversacionEspecialista,
  fetchMensajesEspecialista,
  fetchFichaContactoEspecialista,
  enviarMensajeEspecialista,
  acordarConversacionEspecialista,
  descartarConversacionEspecialista,
  finalizarServicioEspecialista,
  marcarConversacionEspecialistaLeida,
  subscribeMensajesEspecialista,
  subscribeConversacionEspecialista,
  getSignedEspecialistaChatFileUrl,
  esParteAutora,
  chatAbierto,
  formatFechaOferta,
  formatFranjaOferta,
...
const VOLVER = '/especialistas?tab=conversaciones';

export default function ConversacionEspecialista() {
  ...
  useEffect(() => {
    ...
    setLoading(true);
    Promise.all([
      fetchConversacionEspecialista(conversacionId, perfil.id),
      fetchMensajesEspecialista(conversacionId),
    ])
      .then(([c, m]) => {
        if (!activo) return;
        setConversacion(c);
  ...
  useEffect(() => {
    return subscribeMensajesEspecialista(conversacionId, (nuevo) => {
      setMensajes((prev) => (prev.some((m) => m.id === nuevo.id) ? prev : [...prev, nuevo]));
    });
  }, [conversacionId]);

  // La fila de la conversación en vivo: cada parte ve el acuerdo o la
  // finalización de la otra sin recargar.
  useEffect(() => {
    return subscribeConversacionEspecialista(conversacionId, (nueva) => {
  ...
  useEffect(() => {
    ...
    marcarConversacionEspecialistaLeida(conversacion, perfil.id).catch(() => {});
  ...
  useEffect(() => {
    ...
    let activo = true;
    fetchFichaContactoEspecialista(otroId)
      .then((f) => {
  ...
  async function handleEnviar(e) {
    ...
    setError('');
    try {
      await enviarMensajeEspecialista({
        conversacionId,
  ...
  async function handleAcordar() {
    setAcordando(true);
    try {
      const actualizada = await acordarConversacionEspecialista(conversacion, perfil.id);
      setConversacion((prev) => ({ ...prev, ...actualizada }));
      showToast(
        actualizada.estado === 'aceptada'
          ? '¡Servicio confirmado! Sigan coordinando por acá.'
  ...
  async function handleDescartar() {
    try {
      const actualizada = await descartarConversacionEspecialista(conversacionId);
      setConversacion((prev) => ({ ...prev, ...actualizada }));
  ...
  async function handleFinalizar() {
    try {
      await finalizarServicioEspecialista(conversacionId);
      const actualizada = await fetchConversacionEspecialista(conversacionId, perfil.id);
  ...
  const resolverUrlAdjunto = useCallback((path) => getSignedEspecialistaChatFileUrl(path), []);
  ...
  if (loading) {
    return (
      <div className="flex min-h-svh flex-col pb-16">
        <ScreenHeader title="Conversación" fallbackTo={VOLVER} />
    ...
    return (
      <div className="flex min-h-svh flex-col pb-16">
        ...
        <div className="px-5 py-5">
          ...
          <Button className="mt-3" onClick={() => navigate(VOLVER)}>
  ...
  const esTablon = conversacion.origen === 'tablon';
  const oferta = conversacion.oferta;
  const fecha = formatFechaOferta(oferta);
  const franja = formatFranjaOferta(oferta);

  return (
    // pb-16 reserva el alto de la barra inferior, que es `fixed`.
    <div className="flex min-h-svh flex-col pb-16">
      <ScreenHeader title={nombreOtro} fallbackTo={VOLVER} conCampana />
      ...
      <div className="flex flex-col gap-3 px-5 py-4">
        <Card className="flex flex-col gap-2">
          ...
          <p className="text-[11px] text-[#5A6B7A]">
            {esTablon
              ? soyAutora
                ? `${NOMBRE_ESPECIALISTAS} · respondió tu oferta`
                : `${NOMBRE_ESPECIALISTAS} · respondiste su oferta`
              : soyAutora
                ? `${NOMBRE_ESPECIALISTAS} · te contactó desde el directorio`
                : `${NOMBRE_ESPECIALISTAS} · lo contactaste desde el directorio`}
          </p>

          {esTablon ? (
            <>
              <p className="text-[13px] font-medium text-[#0A1628]">
                {labelTipoOferta(oferta?.tipo)}
                {oferta?.especialidad ? ` · ${oferta.especialidad}` : ''}
              </p>
            ...
            <p className="text-[13px] text-[#0A1628]">
              Consulta con especialista. Acuerden acá el alcance, la fecha y el valor del servicio.
            </p>
          )}
        </Card>

        <FichaContactoEspecialista ficha={ficha} />

        {(aceptada || conversacion.estado === 'finalizada') && (
          <Card className="flex flex-col gap-1">
            <p className="text-[12px] font-semibold text-[#0A1628]">Cobro</p>
            <p className="text-[12px] text-[#5A6B7A]">
              El pago se acuerda y se hace por fuera de la app: MUVET no interviene en el cobro entre ustedes.
            </p>
          </Card>
        )}
      </div>
      ...
      {conversacion.estado === 'finalizada' && (
        <div className="sticky bottom-16 border-t border-[#E1E8ED] bg-white px-5 py-3">
          ...
          <p className="mt-1 text-[11px] text-[#5A6B7A]">
            El chat quedó cerrado a mensajes nuevos, pero el historial y los adjuntos se conservan acá.
          </p>
        </div>
      )}
      ...
      <Modal open={confirmandoDescarte} onClose={() => setConfirmandoDescarte(false)} title="Descartar conversación">
        <div className="flex flex-col gap-3">
          <p className="text-[13px] text-[#0A1628]">Se cierra para las dos partes y no se puede reabrir.</p>
      ...
      <Modal open={confirmandoFinal} onClose={() => setConfirmandoFinal(false)} title="Finalizar servicio">
        <div className="flex flex-col gap-3">
          <p className="text-[13px] text-[#0A1628]">
            Da el servicio por cumplido. El chat se cierra a mensajes nuevos, pero el historial queda guardado y podrás
            consultarlo desde tu historial.
          </p>
